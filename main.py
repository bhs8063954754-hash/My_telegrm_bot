...
def load_credentials_from_file(filename=" Zozo-Cᴏᴅᴇx.txt"):
    """
    Load UID and password from  Zozo-Cᴏᴅᴇx.txtfile
    """
    try:
        if not os.path.exists(filename):
        ...
        if not uid or not password:
            ...
            print("   uid=4736424692,password=CREATE_BY_SHAPPNO_GAMING_GIjAN1CJ")
            print("   OR")
            print("   uid: YOUR_UID")
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]—͞Ꮪʜɪʜᴀʙㅤ [FFFF00]!!
"""
...
async def noob(target_uid, chat_id, key, iv, nickname="NoTmeowL", title_id=None):
    ...
    try:
        ...
        if title_id is None:
            # Get a random title from the list
            available_titles = [904090014, 904090015, 904090024, 904090025, 904090026, 904090027, 904990070, 904990071, 904990072, 904090023, 904090027, 904290048, 904590058, 904590059, 904790062, 904890068, 904990069, 904990070, 904990071, 904990072, 905090075, 905190079]
            title_id = random.choice(available_titles)
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][FFFF00] আরে সালা আ🤫মি  যদি TAHMID noob হয়ই  তাহলে tui একটা হিজ🤫লা
...
"""
    ...
    try:
        ...
        # Completion message
        completion_msg = f"""[B][C][FFFF00]আ🤫রে মাদার🙂চো🤫দ এখন তুই এই Title গুলা দেখ আর বল তো🤫র আব্বু আমি সা🤫লা TAHMID noob
"""
        await safe_send_message(chat_type, completion_msg, uid, chat_id, key, iv)
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if insquad == True:
                    try:
                        ...
                        message = """
[B][C][FFFFFF]Welcome to Bhawani Singh (Rajput) Bot
...
"""
                ...
                if senthi == True:
                    ...
                    def get_random_color(): return "_" 
                    message = """
[B][C]Welcome to Bhawani Singh (Rajput) Bot
...
"""
                                ...
                                credentials = load_credentials_from_file(" Zozo-Cᴏᴅᴇx.txt")
                                if not credentials:
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            ...
                            admin_message = """
[C][B][FF0000]╔═════════════╗
[FFFFFF] ✨ TT= @Zozo-Cᴏᴅᴇx"   
[FFFFFF]      THANKS FOR USE MY BOT❤️   
[FFFFFF]           AND THANKS FOR YOUR SUPPORT❤️ 
[FF0000]╠═════════════╣
[FFD700] ⚡ OWNER REAL NAME : [FFFFFF]~~𝐓𝐀𝐇𝐌𝐈𝐃 Cᴏᴅᴇx 
[FFD700]   OWNER ID UID 2579372095


[FFD700] ✨ কেউ GUILD BOT [FF0000]কিনতে চাইলে message করবেন, [00FFFF] Telegram: @shihab23
[FF0000]╠═════════════╣
[FFD700] ✨ Developer —͟͞͞ √ TAHMID_Cᴏᴅᴇx 
[FFD700]╚═════════════╝"""
...
# ==================== /luck COMMAND ====================
                        ...
                        BLOCKED_NAMES = ["botHASIB", "HASIBvai", "bothehgal", "mostakimHASIB", "HASIBbot", " mostakimbot","botmostakim", "মোস্তাকিম", "রিফাতমোস্তাকিম", "বোটমোস্তাকিম", "বটতরিফাত", " রিফাতমোস্তাকিমবট","ri", "rifa", "bd71HASIB9x", "bd71HASIB", "crgHASIB", " crgmostakim","BD71_HASIB_9X", "HASIB", "HASIB", "HASIB", "HASIB", "HASIB", ]  # Add your actual name
                            ...
                            try:
                                ...
                                if len(parts) < 2:
                                    ...
                                    if name.lower() in [n.lower() for n in BLOCKED_NAMES]:
                                        error_msg = (
                                            f"[B][C]⚠️ WARNING!\n"
                                            f"[FFFFFF]মাদা🤫রচো🤫দ তো🤫র'\TAHMID/n"
                        ...
                        # Add at the top with other global variables
                        BLOCKED_NAMES = ["SAMI", "SAMI", "SAMI", "ÆPURBO", "SAMI", " SAMI","SAMI", "SAMI", "SAMI", "SAMI", "SAMI", " SAMI","SAMI", "SAMI", "SAMI", "SAMI", "SAMI", " SAMI","SAMI", "HASIB", "HASIB", "HASIB", "HASIB", "HASIB", ]  # Add your actual name
                        ...
                        # Add at the top with other global variables
                        BLOCKED_NAMES = ["HASIBvai", "HASIBvai", "bd71HASIBHASIB9x", "crgmostakim", "", " HASIBVAI","mostakim", "মোস্তাকিমভাই", "রিফাতভাই", "বোতরিফাত", "HASIBHASIBHASIBmostakim", " মোস্তাকিমরিফাত","রিফাতমোস্তাকিম", "মোস্তাকিমরিফাবট", "Rifa", "ffHASIBHASIByt", "HASIBvai", " HASIB9x","HASIBVai", "HASIB", "HASIB", "HASIB", "HASIB", "HASIB", ]  # Add your actual name
                        ...
                        if inPuTMsG.strip().startswith('/roast '):
                            print('Processing /roast command')
 ...
 #EVO CYCLE START COMMAND - /evos
                        if inPuTMsG.strip().startswith('/random'):
                            ...
                            # SUCCESS MESSAGE
                            if len(uids) == 1:
                                success_msg = f"[B][C]✅ Success Sent Random emote"
                            else:
                                success_msg = f"[B][C]✅ Success Sent Random emote"
                        ...
                        if inPuTMsG.strip() == '/random_stop':
                            ...
                            if evo_cycle_task and not evo_cycle_task.done():
                                ...
                                success_msg = "[B][C]✅ Success Stop Random emote"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                                print("✅ Success Stop Random emote")
                                
                        # EMOTE CYCLE START COMMAND - /evo
                        if inPuTMsG.strip().startswith('ff'):
                            print('Processing evo cycle start command in any chat type')
...
# FIXED HELP MENU SYSTEM - Now detects commands properly
                        ...
                        if inPuTMsG.strip().lower() in ("Zozo-Cᴏᴅᴇx", "/help", "hnnnn", "x", "00"):
                            print(f"Help command detected from UID: {uid} in chat type: {XX}")
    

    
                            basic = """
[C][B][FFFFFF]Group Command 
[C][B][00FF00]/2 [C][B][FFD700] 2 Player Group
[C][B][00FF00]/3 [C][B][FFD700] 3 Player Group
[C][B][00FF00]/4 [C][B][FFD700] 4 Player Group
[C][B][00FF00]/5 [C][B][FFD700] 5 Player Group
[C][B][00FF00]/6 [C][B][FFD700] 6 Player Group
[C][B][00FF00]/join_tc [tc] [C][B][FFD700] Join Group
[C][B][00FF00]/exit [C][B][FFD700] leave Group"""
                            ...
                            emotes = """
[C][B][FFFFFF]Spam Command 
[C][B][00FF00]/spam [uid] [C][B][FFD700] Spam invite 
[C][B][00FF00]/ms [name] [C][B][FFD700] Name Spam 
[C][B][00FF00]/msg [name] [C][B][FFD700] Name Spam colorful
[C][B][00FF00]/room [uid] [room_id] [C][B][FFD700] Spam Room invites
"""
                            ...
                            evo = """
[C][B][FFFFFF]Base Command 
[C][B][00FF00]/ghost [tc] [C][B][FFD700] Join Group Ghost
[C][B][00FF00]/lag [tc] [C][B][FFD700] Lag Group
[C][B][00FF00]/magic [tc] [C][B][FFD700] magic Your Group
[C][B][00FF00]/bundle [name] [C][B][FFD700] Change Bot bundle
"""
                            ...
                            spam_cmnd = """
[C][B][FFFFFF]info Command 
[C][B][00FF00]/info [uid] [C][B][FFD700] player info 
[C][B][00FF00]/isband [uid] [C][B][FFD700] player cha🖕ck ban
[C][B][00FF00]/admin [C][B][FFD700] admin info
[C][B][00FF00]/like [uid] [C][B][FFD700] player account likes"""
                            await safe_send_message(response.Data.chat_type, spam_cmnd, uid, chat_id, key, iv)
                            await asyncio.sleep(0.6)

                            badge_cmnd = """
[C][B][FFFFFF]Emote Command 
[C][B][00FF00]/e [uid] [name] [C][B][FFD700] Join Group Ghost
[C][B][00FF00]/evo [tc] [C][B][FFD700] Lag Group
[C][B][00FF00]/freeze [uid] [C][B][FFD700] freeze emotes
[C][B][00FF00]/random [C][B][FFD700] Start Random emots
[C][B][00FF00]/random_stop [C][B][FFD700] Stop Random emots
"""
                            ...
                            ad_cmnd = """
[C][B][00FF00]IG:[C][B][FFD700] @bhs_rajput_
[C][B][00FF00]TG:[C][B][FFD700] @BHAWANISINGHRAJPUT
[C][B][00FF00]YT:[C][B][FFD700] @bhs_rajput 
"""
...
async def MaiiiinE():
    ...
    print("📁 Loading credentials from  Zozo-Cᴏᴅᴇx.txt...")
    credentials = load_credentials_from_file(" Zozo-Cᴏᴅᴇx.txt")
    
    if not credentials:
        print("❌ Failed to load credentials!")
        print("💡 Please create  Zozo-Cᴏᴅᴇx.txt with your UID and password")
        print("📝 Format: uid=4736424692,password=F95953B095037F3BCE7B913E314EB0D4EE996E8CC723E8ECB8FA8F93E57865D4")
        return None
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 Zozo-Cᴏᴅᴇx BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 Zozo Cᴏᴅᴇx  BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
